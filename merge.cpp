...
int main() {
  std::vector<int> arr = {83, 86, 77, 15, 93, 35, 86, 92, 49, 21, 62, 27, 90, 59, 63, 26, 40, 26, 72, 36};
  ...
  for (int num : arr)
    std::cout << num << " ";
    std::cout << std::endl;
    std::cout << "Time taken by Simple Merge Sort: " << durationSimple.count() << " seconds" <<
    std::endl;
  ...
  mergeSortMultithread(arr, 0, arrSize - 1, 2); // Using 2 threads auto
  endMultithread = std::chrono::high_resolution_clock::now();
  ...
  std::cout << std::endl;
  std::cout << "Time taken by Multithreaded Merge Sort: " << durationMultithread.count() << "
  seconds" << std::endl;
  return 0;
}
